...
if __name__ == "__main__":
    ...
    for i_entry in range(n_evts):
        ...
        for j in range(len(truthinfo)):
            ...
            if x.pdg_code() in [3122]:
                ...
                lambdas+=[[x.p(),pos,[],x.creation_process(),True]]
                # 
            ...
            if x.pdg_code() in [321]:
                ...
                # print("found one", x.pdg_code())
                pos=[x.position().x(),x.position().y(),x.position().z(),x.position().t()]
                kaons+=[[x.p(),pos,[],x.creation_process(),True]]
            ...
            if y.ancestor_pdg_code()==321:
                ...
                if kaons[0][1]==pos2:
                    ...
                    if y.parent_pdg_code!=2112:
                        kaons[0][-1]*=is_contained(pos3)
                        kaons[0][-1]*=is_contained(pos4)
    ...
    np.save(SAVEDIR,np.array([final_lambda,final_kaon], dtype=object))
